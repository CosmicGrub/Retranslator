...
object TranslationEngine {
    ...
    fun downloadModel(context: Context, code: String, requireWifi: Boolean, onDone: (Boolean, String?) -> Unit) {
        val conditions = if (requireWifi && !DownloadManager.allowCellularDownloads(context)) DownloadConditions.Builder().requireWifi().build() else DownloadConditions.Builder().build()
     ...
     *
     * fold5-device-version branch: [DownloadManager.allowCellularDownloads]
     * defaults `true` here, so by default this whole Wi-Fi gate (and the
     * `requireWifi()` conditions below) is bypassed - a missing model
     * downloads over cellular too - but it's a real Settings toggle
     * (Settings > Language packs), not a hardcoded constant, so a user who
     * turns it off gets this method's original Wi-Fi-gated behavior back
     * exactly. See that function's doc comment for why this is scoped to
     * this branch specifically and why it doesn't reopen the bug this
    ...
    private fun attemptTranslate(
        ...
        if (!bothReady && !DownloadManager.allowCellularDownloads(context) && !DownloadManager.isOnWifi(context)) {
            onError(
        ...
        val conditions = if (DownloadManager.allowCellularDownloads(context)) DownloadConditions.Builder().build() else DownloadConditions.Builder().requireWifi().build()
