 ...
 * Vendored from the espeak-ng project (android/src/com/reecedunn/espeak)
 * for direct in-app bundling; the JNI symbol names exported by
 * libttsespeak.so are tied to this exact package/class name, so it is kept
 * unmodified apart from removing the getSampleText() helper (which pulled
 * in an Android string resource this app does not ship).
 */
