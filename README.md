...
- **Not yet built**: Spanish/German/French course content (the pipeline above supports it — drop a `<langCode>_course.json` in `assets/learn/` in the same shape — but no additional language's content has been authored or verified yet), and a dedicated "due for review" queue UI that resurfaces items by their Leitner-box schedule (the SRS *data* is tracked and persisted correctly today; a screen that surfaces it hasn't been built).
