...
Status: **partially implemented, partially verified**. Originally captured decisions from a brainstorming session on 2026-08-09 with no code written against it yet; that is no longer true. As of 2026-08-10: §2/§3 (Conversations mirrored + cover layouts) and §4 (dual-recognizer auto-detect + continuous listening) are built and carry their own verification status inline; §6 (settings-driven layout variants for Translate/Practice/Learn, added 2026-08-10 despite the Scope table below) is built, including a picker-UI bug that was found and fixed the same day, fix verified on-device — see §6 for specifics. **No pass in this spec's history has run against the actual target device, RFCW80CK2RW** — every on-device claim anywhere in this document is from a Retroid Pocket 2+ substitute (serial `15780287351340`), except §4's original dual-recognizer prototype numbers, which predate this note and were captured on the real Fold 5 before it became unavailable to later sessions. Read every "verified on-device" claim below with that in mind.
